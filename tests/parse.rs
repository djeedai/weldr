...
use weldr::{
  FileRefResolver, SourceFile, SubFileRef,
  CommandType
};
...
impl MemoryResolver {
  fn new() -> MemoryResolver {
    MemoryResolver{ file_map: HashMap::new() }
  }
...
fn get_resolved_subfile_ref(cmd: &CommandType) -> Option<Rc<RefCell<SourceFile>>> {
  match cmd {
    CommandType::SubFileRef(sfr_cmd) => {
      match &sfr_cmd.file {
...
fn parse_recursive() {
  ...
  memory_resolver.add("b.ldr", b"4 16 1 1 0 0.9239 1 0.3827 0.9239 0 0.3827 1 0 0\n1 16 0 0 0 1 0 0 0 1 0 0 0 1 a.ldr");
